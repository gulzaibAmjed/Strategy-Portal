...
var WYSIWYG = {
	...
	updateStatusBar: function(n) {
		...
		// update the status bar 	
		var statusbar = $_("wysiwyg_statusbar_" + n);
		if(statusbar){ 
	...
	writeDropDown: function(n, id) {
		...
		output += '<td onMouseOver="$_(\'img_' + dropdown.id + '_' + n + '\').src=\'' + imageOn + '\';" onMouseOut="$_(\'img_' + dropdown.id + '_' + n + '\').src=\'' + image + '\';">';
	...
	closeDropDowns: function(n, exid) {
		...
		for(var id in dropdowns) {
			...
			if(dropdown.id != exid) {
				...
				if($_(divId)) $_(divId).style.display = 'none';
			}
		}			
	},
	...
	openDropDown: function(n, id) {
		...
		if($_(divId).style.display == "none") {	
			$_(divId).style.display = "block"; 
		}
		else {
			$_(divId).style.display = "none"; 
		}
		$_(divId).style.position = "absolute";
	...
	viewSource: function(n) {
		...
		if($_('HTMLMode' + n)) {
		    $_('HTMLMode' + n).style.display = 'none'; 
		}
	    if($_('textMode' + n)) {
		    $_('textMode' + n).style.display = 'block';
		}
	...
	viewText: function(n) { 
		...
		if($_('HTMLMode' + n)) {
			$_('HTMLMode' + n).style.display = 'block';
		}
